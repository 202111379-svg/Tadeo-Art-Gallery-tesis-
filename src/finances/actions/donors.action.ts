...
const col = (uid: string) =>
  collection(FirebaseDB, `${uid}/gallery/donors`);

export const getDonorsAction = async (uid: string): Promise<Donor[]> => {
  const snap = await getDocs(col(uid));
  return snap.docs.map((d) => ({ ...(d.data() as Donor), id: d.id }));
};

export const addDonorAction = async (
  uid: string,
  donor: Omit<Donor, 'id'>
): Promise<Donor> => {
...
export const deleteDonorAction = async (
  uid: string,
  id: string
): Promise<void> => {
